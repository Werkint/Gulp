...
var stripBundleSuffix = function (name) {
    return name.replace(/Bundle$/, '');
};

var App = function (data, views, twigGlobals) {
            ...
            if (node.data) {
                ...
                if (node.name.match(/\.twig\.json$/)) {
                    var pageName = node.name.split(/\./).slice(-3, -1).join('.'),
                      bundle = stripBundleSuffix(bundleName),
                      key = '@' + bundle + prefixIn + '/' + pageName;
        ...
        _.each(ns.children, function (dir) {
            if (dir.name === 'Bundle') {
                _.each(dir.children, function (bundle) {
                    ...
                    var bundleName = ns.name +
                      bundle.name.replace(/Bundle$/, '');

                    loadViews('@' + bundleName, '', row.children);
                });
            }
        });
    });
...
App.prototype.loadTwig = function () {
    ...
    this.twig.extendFunction('asset', function (val) {
        return '/' + val;
    });
